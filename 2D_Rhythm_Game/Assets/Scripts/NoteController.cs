...
{
    //하나의 노트에 대한 정보를 담는 노트(note) 클레스를 정의
    class Note
    {
    ...
    void MakeNote(Note note) //노트가 만들어 졌을 때 로직
    {
    ...
    IEnumerator AwitWakeNote(Note note)
    {
    ...
    void Start()
    {
        ...
        TextAsset textAsset = Resources.Load<TextAsset>("Beats/" + PlayerInformation.seletedMusic);
        StringReader reader = new StringReader(textAsset.text);
        //첫 번째 줄에 적힌 곡 이름을 읽습니다.
        musicTitle = reader.ReadLine();
        //두 번째 줄에 적힌 아티스트 이름을 읽습니다.
        musicArtist = reader.ReadLine();
        //세 번째 줄에 적힌 비트 정보(BPM, Divider, 시작 시간)을 읽습니다.
        string beatInformation = reader.ReadLine();
        bpm = Convert.ToInt32(beatInformation.Split(' ')[0]); //160 값 //공백을 기준으로 읽어들여옴 >> "160 30 3.5"이것을
        divider = Convert.ToInt32(beatInformation.Split(' ')[1]); //30 값
        startingPoint = (float) Convert.ToDouble(beatInformation.Split(' ')[2]); //3.5 값
        // 1초마다 떨어지는 비트 개수
        ...
        {
            StartCoroutine(AwitWakeNote(notes[i]));
        }
